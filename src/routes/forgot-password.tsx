...
import { Mail, ArrowRight, Sparkles, LifeBuoy } from 'lucide-react'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErr(data.detail || 'Password reset request failed. Please try again.')
      }
    } catch (error) {
      console.error('[Equinox UI forgot password error]', error)
...
<img src="/logo.png" alt="Equinox Core" className="h-10 w-auto object-contain invert dark:invert-0" />
        ...
        <div className="relative z-10 space-y-6 my-auto text-left">
          ...
          <h2 className="text-4xl font-black tracking-tight leading-[1.08] text-[var(--sea-ink)]">
            Locked out of the terminal? Let's reset.
          </h2>
          <p className="text-sm text-[var(--sea-ink-soft)] font-medium max-w-sm leading-relaxed">
            Submit your registered email address below, and we will send a 6-digit OTP verification code to securely restore your session.
          </p>
        </div>
        ...
        <Card className="relative z-10 rounded-2xl border border-[var(--line)] bg-[var(--surface-strong)] p-5 shadow backdrop-blur-md text-left flex flex-row items-start gap-4 space-y-0">
          <div className="h-10 w-10 rounded-xl bg-[var(--chip-bg)] border border-[var(--line)] flex items-center justify-center shrink-0">
            <LifeBuoy className="h-5 w-5 text-[var(--sea-ink)]" />
          </div>
          <div>
            <h4 className="text-xs font-bold text-[var(--sea-ink)]">Advisory Support</h4>
            <p className="text-[11px] text-[var(--sea-ink-soft)] mt-1 leading-normal">
              Need assistance? Check that the SMTP email worker settings are configured correctly on your backend application environment.
            </p>
          </div>
        </Card>
      </section>
...
<img src="/logo.png" alt="Equinox" className="h-8 w-auto object-contain invert dark:invert-0" />
          ...
          <div className="text-left space-y-2">
            <h1 className="text-3xl font-black text-[var(--sea-ink)] tracking-tight">Reset Password</h1>
            <p className="text-xs text-[var(--sea-ink-soft)] font-medium">Request a 6-digit OTP to change your credentials</p>
          </div>
