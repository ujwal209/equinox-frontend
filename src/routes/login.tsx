    ...
    try {
      ...
      if (res.ok && data.access_token) {
        ...
        if (success) {
          ...
          setErr('Login succeeded, but failed to load user profile.')
        }
      } else {
        setErr(data.detail || 'Incorrect email credentials or password.')
      }
    } catch (error) {
      console.error('[Equinox UI Login Error]', error)
...
<img src="/logo.png" alt="Equinox Core" className="h-10 w-auto object-contain invert dark:invert-0" />
        ...
        <div className="relative z-10 space-y-6 my-auto text-left">
          <Badge variant="outline" className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-[var(--sea-ink-soft)] bg-[var(--chip-bg)] border border-[var(--line)] px-3 py-1 rounded-full h-auto">
            ...
            SaaS suggestion platform
          </Badge>
          <h2 className="text-4xl font-black tracking-tight leading-[1.08] text-[var(--sea-ink)]">
            Algorithmic target calculations, in real-time.
          </h2>
          <p className="text-sm text-[var(--sea-ink-soft)] font-medium max-w-sm leading-relaxed">
            Monitor mock timeseries channels, analyze consensus signals, and calculate optimal stop-loss ratios from a single terminal feed.
          </p>
        </div>
        ...
        <Card className="relative z-10 rounded-2xl border border-[var(--line)] bg-[var(--surface-strong)] p-5 shadow backdrop-blur-md text-left flex flex-row items-start gap-4 space-y-0">
          <div className="h-10 w-10 rounded-xl bg-[var(--chip-bg)] border border-[var(--line)] flex items-center justify-center shrink-0">
            <TrendingUp className="h-5 w-5 text-[var(--sea-ink)]" />
          </div>
          <div>
            <h4 className="text-xs font-bold text-[var(--sea-ink)]">Surveillance Metric Tracker</h4>
            <p className="text-[11px] text-[var(--sea-ink-soft)] mt-1 leading-normal">
              Continuous random-walk price simulation and consolidated buy-hold analyst consensus mappings for foreign & Indian equity classes.
            </p>
          </div>
        </Card>
      </section>
...
<img src="/logo.png" alt="Equinox" className="h-8 w-auto object-contain invert dark:invert-0" />
          ...
          <div className="text-left space-y-2">
            <h1 className="text-3xl font-black text-[var(--sea-ink)] tracking-tight">Access Terminal</h1>
            <p className="text-xs text-[var(--sea-ink-soft)] font-medium">Please enter your email credentials to login</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <Link
                  to="/forgot-password"
                  className="text-[10px] font-bold text-[var(--sea-ink-soft)] hover:text-[var(--sea-ink)] no-underline transition"
                >
                  Forgot Password?
                </Link>
              </div>
