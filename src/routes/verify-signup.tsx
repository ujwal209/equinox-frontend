...
import { Check, Mail, Key, Sparkles, FileInput } from 'lucide-react'
...
function VerifySignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailInput.trim() || otp.length !== 6) {
      setErr('Please enter a valid email and 6-digit OTP.')
      return
    }
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/v1/auth/verify-signup`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: emailInput.trim(), otp: otp.trim() })
      })
      ...
      if (res.ok) {
        ...
        setErr(data.detail || 'Incorrect or expired OTP verification code.')
      }
    } catch (error) {
      console.error('[Equinox UI OTP verification error]', error)
...
<img src="/logo.png" alt="Equinox Core" className="h-10 w-auto object-contain invert dark:invert-0" />
        ...
        <div className="relative z-10 space-y-6 my-auto text-left">
          <Badge variant="outline" className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-[var(--sea-ink-soft)] bg-[var(--chip-bg)] border border-[var(--line)] px-3 py-1 rounded-full h-auto">
            ...
            Active Surveillance Verification
          </Badge>
          <h2 className="text-4xl font-black tracking-tight leading-[1.08] text-[var(--sea-ink)]">
            Verify email verification parameters.
          </h2>
          <p className="text-sm text-[var(--sea-ink-soft)] font-medium max-w-sm leading-relaxed">
            Enter the 6-digit validation code dispatched to your registration inbox to activate your account.
          </p>
        </div>
        ...
        <Card className="relative z-10 rounded-2xl border border-[var(--line)] bg-[var(--surface-strong)] p-5 shadow backdrop-blur-md text-left flex flex-row items-start gap-4 space-y-0">
          <div className="h-10 w-10 rounded-xl bg-[var(--chip-bg)] border border-[var(--line)] flex items-center justify-center shrink-0">
            <FileInput className="h-5 w-5 text-[var(--sea-ink)]" />
          </div>
          <div>
            <h4 className="text-xs font-bold text-[var(--sea-ink)]">Input Parameters</h4>
            <p className="text-[11px] text-[var(--sea-ink-soft)] mt-1 leading-normal">
              If SMTP values are not active inside the environment file, copy the verification code from your terminal execution logs.
            </p>
          </div>
        </Card>
      </section>
...
<img src="/logo.png" alt="Equinox" className="h-8 w-auto object-contain invert dark:invert-0" />
          ...
          <div className="text-left space-y-2">
            <h1 className="text-3xl font-black text-[var(--sea-ink)] tracking-tight">Verify Email</h1>
            <p className="text-xs text-[var(--sea-ink-soft)] font-medium">Verify your email address using OTP code</p>
          </div>
          ...
          <div className="border-t border-[var(--line)] pt-4 text-center">
            <p className="text-[11px] text-[var(--sea-ink-soft)] font-medium">
              Want to request another code?{' '}
              <Link to="/signup" className="text-[var(--sea-ink)] hover:underline no-underline font-bold">
                Re-Register
              </Link>
            </p>
          </div>
