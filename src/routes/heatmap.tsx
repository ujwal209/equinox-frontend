...
function DashboardHeatmap() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300 w-full max-w-7xl mx-auto pb-24 px-4 sm:px-6 pt-10 text-left">
      ...
      {!loading && sectors.length > 0 && selectedSector && (
        <div className="space-y-12">
          {sectors
            .filter((sec) => sec.sector === selectedSector)
            .map((sec) => (
              <SectorCard key={sec.sector} sectorData={sec} sortOrder={sortOrder} />
            ))}
        </div>
      )}
    </div>
...
function SectorCard({ sectorData, sortOrder }: { sectorData: HeatmapSector; sortOrder: 'perf-high' | 'perf-low' | 'name' }) {
  ...
  return (
    <Card className="border-border bg-muted/10 rounded-[2.5rem] p-8 text-left flex flex-col gap-6 shadow-xl relative overflow-hidden">
        ...
        {sortedCompanies.map((c) => {
          ...
          if (loading || !c.isLoaded) {
            return (
              ...
              >
                <div className="h-5 w-14 bg-zinc-855 rounded" />
                <div className="h-4 w-20 bg-zinc-855 rounded self-end mt-4" />
              </div>
            )
          }
            ...
            >
              <div className="flex items-center justify-between gap-2">
                {logoUrl ? (
                  ...
                  <div className="h-10 w-10 rounded-2xl bg-zinc-800 flex items-center justify-center text-[10px] font-black text-foreground shrink-0 uppercase">
                    {c.symbol.slice(0, 2)}
                  </div>
                )}
